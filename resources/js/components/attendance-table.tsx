...
import { Upload, Search, UserSearch, Download, Trash2 } from "lucide-react";
...
import * as attendanceManagement from "@/routes/admin/attendance-management";
...
export function AttendanceTable({
    ...
    const handleFileChange = (event: ChangeEvent<HTMLInputElement>): void => {
        ...
        setIsImporting(true);
        router.post(attendanceManagement.importCsv().url, { attendance_csv: file }, {
            forceFormData: true,
            preserveScroll: true,
            onSuccess: () => {
    ...
    const handleExportClick = (): void => {
        window.location.href = attendanceManagement.exportCsv.url({
            query: {
                search: searchTerm,
            },
        });
    ...
    const handleClearImportedClick = (): void => {
        const confirmed = window.confirm("Clear all imported attendance records?");
        if (!confirmed) {
            return;
        }

        setIsClearing(true);
        router.delete("/admin/attendance-management/clear-imported", {
            preserveScroll: true,
            onSuccess: () => {
                toast.success("Imported attendance records cleared.");
            },
            onError: () => {
                toast.error("Failed to clear imported attendance records.");
            },
            onFinish: () => {
                setIsClearing(false);
            },
